---
title: "Pragmatics in CTSL - Subjectivity and Faultless Disagreement (TURKISH)"
...
# load datasets
subjectivity = read.csv("../data/turkish-subjectivity-trials.csv", header = TRUE) %>%
  filter(trial_type!="bot_check") %>%
  mutate(experiment = "subjectivity")

disagreement = read.csv("../../../05_faultless_disagreement/turkish/data/turkish-disagreement-trials.csv", header = TRUE) %>%
  filter(trial_type != "bot_check" ) %>%
...
cor.test(d_wide$subjectivity, d_wide$disagreement, method = "pearson") # r = 0.92, 95% CI = [0.86, 0.96]
